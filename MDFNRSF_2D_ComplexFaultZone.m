...
DomainMinX=0
DomainMaxX=40000
DomainCenter=[(DomainMaxX-DomainMinX)/2,(DomainMaxY-DomainMinY)/2];
% LoadingRate=1e-11; %m/s
LoadingRate=0;%0.1e6/60/60/24/365; %Pa/s
Sigma1=30e6; % Currently equals to loading direction
Sigma3=10e6;

% Generate Complex Fault zone
[FaultElementCenter,FaultElementLength,FaultAngle,FaultAngleRad,...
    FaultRRLL,InitialNormalStress,InitialShearStress,...
    VlStress,FaultElementCount,SegmentCountPerFault,FaultLength_Overall,...
    FaultNumberForElement,FaultLength_Bulk,FaultCenter_Bulk]...
    =Function_FaultGeneration( FaultCount,FaultAngle1,FaultAngle2,FaultLengthMin,...
    FaultLengthMax,DomainMinX,DomainMaxX, DomainMinY,DomainMaxY,Sigma1,Sigma3 ,LoadingRate);
